...
Deno.serve(async (req) => {
  ...
  if (appUser?.role !== 'sponsor' && !(await requireAdmin(actor.id))) return fail('forbidden', 'Sponsor or admin role required', 403);
  const { data, error } = await db.from('sponsorship_requests').select('*, learner_profiles(education_level), users!sponsorship_requests_learner_id_fkey(full_name)').eq('status', 'pending').order('created_at', { ascending: false });

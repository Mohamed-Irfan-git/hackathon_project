...
Deno.serve(async (req) => {
  ...
  const { data, error } = await admin().from('sponsorship_requests').insert({ learner_id: actor.id, opportunity_id: input.opportunity_id ?? null, title: input.title.trim(), reason: input.reason.trim(), amount_needed: input.amount_needed }).select('id,status').single();
